...
curl -sS http://127.0.0.1:8093/api/v1/profiles \
  ...
  -d '{"name":"赤石北坡剖面","site":"赤石岭北侧","depth_mm":10000,"note":"砂泥岩层序","recorder":"王工"}'
```
...
| `GET /rocks` | 岩性代码与中文名称 |
| `POST /profiles` | `name, site, depth_mm, note, recorder`（`recorder` 可省略） |
...
单个剖面最多 500 层、500 个历史版本，总深度最大 1000000 毫米。最多 2000 个剖面、10000 个对比结果，总快照上限 64 MiB。岩性支持 `sandstone / mudstone / limestone / shale / conglomerate / unknown`。名称最多 120 字、地点 200 字、说明 2000 字、编录人 120 字（可省略），单层描述 1000 字，标志层名称 80 字，修订理由 1–500 字。标志层名称在同一剖面内忽略大小写后必须唯一。
...
快照包含 SHA-256 校验值。启动时检查校验值、版本连续性、状态转换及对比可重复性，遇到损坏拒绝启动。快照数据带有 `schema` 版本号，当前版本为 2（剖面元数据含 `recorder`）。启动时按版本分派解码：版本 1 的旧快照按当时结构严格解码、校验，并在内存中迁移为当前形态（`recorder` 置空），首次成功写入后以新格式落盘；读取接口不变，调用方无需感知两种形态。每种版本按各自结构校验，快照出现未知版本号或未知字段一律拒绝启动，不会静默丢弃字段。进程在替换前中断保留旧快照，替换后中断使用新快照。同目录遗留的 `.strata-*` 临时文件不会参与恢复，可在服务停止时清理。若替换后同步目录失败，服务保留新内存状态并停止后续写入，健康状态变为 503；检查磁盘并重启后再读取版本确认结果，不要盲目重放修改。
...
验证覆盖编录成功与深度失败边界、锁定与重新打开、并发版本冲突、历史不变性、数据目录独占、重启恢复、区间相似度、未知岩性、偏移建议、CSV、列表筛选与分页、快照格式迁移与未知字段拒绝。
...
internal/persistence/快照版本校验与迁移、原子替换、独占锁
