...
同一地质专题采用的锁定剖面版本可以整理成集合。集合包含名称、说明、有序成员数组和每个成员的用途说明：

```json
{
  "name": "赤石岭对比专题",
  "description": "统一对比基准层序",
  "members": [
    {"profile_id": "<profile-id-a>", "version": 3, "purpose": "西段标准层序"},
    {"profile_id": "<profile-id-b>", "version": 3, "purpose": "东段对比层序"}
  ]
}
```

将以上对象以 JSON 提交至 `POST /api/v1/collections`，响应为 HTTP 201 和 `version: 1`。成员绑定**具体历史版本**且该版本必须已经锁定；历史版本不可变，剖面重新打开修订后集合仍指向原版本，不会漂移。同一剖面的不同版本可以作为不同成员，同一版本不能重复加入。

集合有独立的版本号：`PUT /api/v1/collections/{id}` 整体替换名称、说明和成员，`DELETE /api/v1/collections/{id}` 删除集合，两者都必须携带当前 `expected_version`，并发修改只有一个成功，其余得到 HTTP 409。删除只移除组织关系，剖面、历史版本和已有对比结果不受影响。
...
| `POST /collections` | `name, description, members` |
| `GET /collections` | `q, profile_id, offset, limit` 筛选与分页 |
| `GET /collections/{id}` | 集合完整内容 |
| `PUT /collections/{id}` | `expected_version, name, description, members` 整体替换 |
| `DELETE /collections/{id}` | `expected_version`，仅移除组织关系 |
...
单个剖面最多 500 层、500 个历史版本，总深度最大 1000000 毫米。最多 2000 个剖面、10000 个对比结果、500 个集合，每个集合最多 200 个成员，总快照上限 64 MiB。岩性支持 `sandstone / mudstone / limestone / shale / conglomerate / unknown`。名称最多 120 字、地点 200 字、说明 2000 字，单层描述 1000 字，标志层名称 80 字，修订理由 1–500 字，成员用途说明 1–500 字。标志层名称在同一剖面内忽略大小写后必须唯一。
...
该存储方案针对小规模资料，使用整份快照和内存副本，读写开销随历史体积增长。剖面、历史版本和对比结果不提供删除；仅集合可以删除，且删除只移除组织关系。未实现自动清理历史、备份轮替或多进程共享。备份时停止服务后复制数据目录；不要人工修改快照内容。
...
**测试模式为 `deferred`**：当前初始化基线有意不生成单元测试、测试数据或专用测试套件；后续“代码测试”任务补充这些内容。`scripts/smoke.py` 是有超时的运行验证入口，它在临时目录编译并启动真实 HTTP 服务、通过本机回环 HTTP 连接完成操作，然后关闭服务并清理临时数据。不会访问外网或修改现有数据目录。也可分别运行 `record / seal / compare / browse / collect` 五个流程。

验证覆盖编录成功与深度失败边界、锁定与重新打开、并发版本冲突、历史不变性、数据目录独占、重启恢复、区间相似度、未知岩性、偏移建议、CSV、列表筛选与分页、集合成员锁定绑定与删除隔离。
