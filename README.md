...
元数据修改与分层替换在规范化后若与当前内容没有业务差异（分层仅数组顺序不同，或文字仅首尾空白不同），接口直接返回当前剖面，不写事件、不增加版本、也不重写快照；响应状态仍为 200。判等在规范化之后进行：深度、岩性、描述和标志层名称的任何不同（包括标志层大小写）都算业务变化，仍会产生新版本。过期版本、剖面锁定和无效输入的拒绝规则优先于去重，内容相同不会绕过这些约束。
