...
| `GET /rocks` | 岩性代码与中文名称 |
| `POST /profiles` | `name, site, depth_mm, note, source` |
...
单个剖面最多 500 层、500 个历史版本，总深度最大 1000000 毫米。最多 2000 个剖面、10000 个对比结果，总快照上限 64 MiB。岩性支持 `sandstone / mudstone / limestone / shale / conglomerate / unknown`。名称最多 120 字、地点 200 字、说明 2000 字、资料来源 200 字，单层描述 1000 字，标志层名称 80 字，修订理由 1–500 字。标志层名称在同一剖面内忽略大小写后必须唯一。
...
快照数据带有结构版本号，当前写入版本为 2（剖面元数据含 `source` 资料来源字段）。版本 1 的旧快照可以直接启动：读取时按版本 1 的结构校验并迁移到内存中的当前形态，迁移后旧剖面的 `source` 为空，表示未记录；下一次成功写入落盘即为版本 2。两种版本各自按自己的结构严格解码，出现未知字段或未知版本号一律拒绝启动，不会静默丢弃数据。
...
**测试模式为 `deferred`**：当前初始化基线有意不生成单元测试、测试数据或专用测试套件；后续“代码测试”任务补充这些内容。`scripts/smoke.py` 是有超时的运行验证入口，它在临时目录编译并启动真实 HTTP 服务、通过本机回环 HTTP 连接完成操作，然后关闭服务并清理临时数据。不会访问外网或修改现有数据目录。也可分别运行 `record / seal / compare / browse / migrate` 五个流程，其中 `migrate` 用手工构造的版本 1 快照验证格式升级与严格解码。
