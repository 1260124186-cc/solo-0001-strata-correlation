...
区域状态为 `pass / fail / blocked / warn`：前置阶段（读取、容量、封装、校验值）失败时，依赖解析数据的版本链与对比引用区域标记为 `blocked`。结论中的定位信息只有剖面/对比编号与版本号（如 `prf_…#v3`、`cmp_…`），字段名使用校验路径（如 `layers[1]`），不输出剖面名称、地点、说明、分层描述、标志层名称等资料正文。诊断绝不修复、重命名或覆盖快照，也不清理临时文件；恢复手段仍是停止服务后从备份复制数据目录。
...
验证覆盖编录成功与深度失败边界、锁定与重新打开、并发版本冲突、历史不变性、数据目录独占、重启恢复、区间相似度、未知岩性、偏移建议、CSV、列表筛选与分页。`diagnose` 流程额外覆盖只读诊断在空目录、健康快照、校验值损坏、版本链语义损坏下的结论一致性（与正常启动同一判断）、临时文件告警、正文不泄露和持锁互斥。
