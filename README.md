...
对比结果只持久化对比参数、算法版本和一份内容摘要（区间数、标志层对数、各长度合计、相似度及覆盖全部对外字段的 SHA-256 内容摘要）；逐段区间和标志层明细不落盘，在列表、详情、CSV 和启动校验时从引用的锁定历史版本用同一算法重新计算，并用内容摘要校验重算结果与当初返回完全一致。检测到不一致时返回内部故障而不是给出另一份内容；损坏的引用快照按启动错误处理。早先保存的含逐段明细快照仍可直接读取，并在该结果再次被复用时自动压缩为新格式，无需重新生成。

快照包含 SHA-256 校验值。启动时检查校验值、版本连续性、状态转换，并逐条重算对比结果核对内容摘要，遇到损坏拒绝启动。进程在替换前中断保留旧快照，替换后中断使用新快照。同目录遗留的 `.strata-*` 临时文件不会参与恢复，可在服务停止时清理。写入会在创建任何临时文件之前先计算快照体积，超出 64 MiB 上限时以 409 拒绝本次写入，旧快照和服务内存状态均不变；不会出现先落盘超限文件再报错。若替换后同步目录失败，服务保留新内存状态并停止后续写入，健康状态变为 503；检查磁盘并重启后再读取版本确认结果，不要盲目重放修改。
