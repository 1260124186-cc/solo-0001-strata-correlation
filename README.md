...
| `POST /profiles/{id}/archive` | `through_version`，归档该版本及更早的历史版本 |
| `GET /profiles/{id}/history` | 按版本升序列出未归档事件，支持 `offset, limit` |
| `GET /profiles/{id}/revisions/{version}` | 指定历史版本及事件，含已归档版本 |
...
单个剖面最多 500 层、500 个未归档的历史版本，总深度最大 1000000 毫米。最多 2000 个剖面、10000 个对比结果，总快照上限 64 MiB。岩性支持 `sandstone / mudstone / limestone / shale / conglomerate / unknown`。名称最多 120 字、地点 200 字、说明 2000 字，单层描述 1000 字，标志层名称 80 字，修订理由 1–500 字。标志层名称在同一剖面内忽略大小写后必须唯一。

差异接口按完整深度区间匹配分层；边界变化展示为原区间移除和新区间增加，相同区间中的岩性或描述修改展示前后值。深度查询使用左闭右开区间，边界点属于其下方分层，剖面底端不属于任何层。

## 历史版本归档

长期剖面的历史会持续增长。`POST /api/v1/profiles/{id}/archive` 把最旧的一段连续版本（第 1 版到 `through_version`）从常规历史移入归档区：

```bash
curl -sS -X POST "http://127.0.0.1:8093/api/v1/profiles/<profile-id>/archive" \
  -H 'Content-Type: application/json' \
  -d '{"through_version":40}'
```

- 归档只接受最旧版本的连续前缀，且最新版本必须保留在历史中；重复归档同一前缀返回 HTTP 409。
- 归档后的版本不再出现在 `GET /profiles/{id}/history`（响应中的 `archived` 字段给出已归档数量），但仍可通过 `GET /profiles/{id}/revisions/{version}` 按原版本号读取，`/at`、`/diff` 与对比接口同样接受归档版本号。
- 版本号是数据而不是位置：归档不会重排任何编号，新版本号在归档前缀之后继续递增。已保存的对比结果引用的版本永久有效。
- 单个剖面最多保留 500 个**未归档**版本；达到上限时先归档更早的版本再继续修订。归档版本不计入该上限。
- 归档区与现行历史、对比结果处于同一快照，重启校验一并检查归档链的连续性。
...
快照包含 SHA-256 校验值。启动时检查校验值、版本连续性（含归档区与现行历史的衔接）、状态转换及对比可重复性，遇到损坏拒绝启动。进程在替换前中断保留旧快照，替换后中断使用新快照。同目录遗留的 `.strata-*` 临时文件不会参与恢复，可在服务停止时清理。若替换后同步目录失败，服务保留新内存状态并停止后续写入，健康状态变为 503；检查磁盘并重启后再读取版本确认结果，不要盲目重放修改。
...
**测试模式为 `deferred`**：当前初始化基线有意不生成单元测试、测试数据或专用测试套件；后续“代码测试”任务补充这些内容。`scripts/smoke.py` 是有超时的运行验证入口，它在临时目录编译并启动真实 HTTP 服务、通过本机回环 HTTP 连接完成操作，然后关闭服务并清理临时数据。不会访问外网或修改现有数据目录。也可分别运行 `record / seal / compare / browse / archive` 五个流程。

验证覆盖编录成功与深度失败边界、锁定与重新打开、并发版本冲突、历史不变性、数据目录独占、重启恢复、区间相似度、未知岩性、偏移建议、CSV、列表筛选与分页、历史归档与归档后读取。
