...
| `-diag` | `STRATA_DIAG` | `false` | 只读诊断启动，输出结构化报告后退出，不监听端口 |

显式参数优先于环境变量。数据目录只能由一个服务进程打开；系统文件锁在进程退出后自动释放。默认只监听本机，适合受信任的单机部署。当前基线未包含身份验证、TLS 和跨节点复制。

## 只读诊断启动

快照损坏会让正常启动直接失败。排查现场可先运行只读诊断，它与正常启动**使用同一把数据目录锁和同一套校验代码**，只输出结构化 JSON 结论，不监听端口、不提供任何写接口：

```bash
go run ./cmd/stratad -diag -data ./data
# 或
STRATA_DIAG=1 go run ./cmd/stratad -data ./data
```

退出码：`0` 表示数据可启动；`1` 表示诊断执行成功但判定快照不可启动；`2` 表示诊断本身无法执行（例如数据目录已被其他进程独占、参数错误）。报告字段：

| 字段 | 含义 |
| --- | --- |
| `startable` | 与正常启动完全一致的结论：`true` 时正常启动不会再因同一问题失败 |
| `snapshot` | 快照是否存在、字节数、摘要算法与 SHA-256 校验结果 |
| `counts` | 剖面数、历史版本总数、对比结果数（不含任何正文） |
| `areas` | 四个检查区域：`snapshot_checksum`、`version_chain`、`comparison_references`、`recoverability` |
| `leftover_temp_files` | 遗留 `.strata-*` 临时文件数（不参与恢复，仅提示停止服务后清理） |
| `recoverability` | 是否可直接启动；不可启动时 `auto_recoverable=false`，不会自动修复或覆盖数据 |

区域状态为 `pass / fail / warn / blocked`：任一 `severity:"error"` 的结论即 `fail`；只有非致命 `warning`（目前仅遗留临时文件）为 `warn`；因前置阶段（读取、容量、封装、校验值、JSON 解析）失败或在版本链闸口短路（如不受支持的快照结构）而**实际未执行**的区域标记为 `blocked`，绝不会把未检查的区域报成 `pass`。每个结论带稳定 `code`、`severity` 和固定中文 `detail`。致命校验失败时 `recoverability` 区域始终保持 `fail`（含一条 `snapshot_unstartable` 的 error 结论）；遗留临时文件只能作为同一区域的附加 `warning` 出现，不能把 `fail` 降级成 `warn`，也不改变 `startable` 结论。结论中的定位信息只有剖面/对比编号与版本号（如 `prf_…#v3`、`cmp_…`），字段名使用校验路径（如 `layers[1]`），不输出剖面名称、地点、说明、分层描述、标志层名称等资料正文。诊断绝不修复、重命名或覆盖快照，也不清理临时文件；恢复手段仍是停止服务后从备份复制数据目录。
...
快照包含 SHA-256 校验值。启动时检查校验值、版本连续性、状态转换及对比可重复性，遇到损坏拒绝启动。诊断启动（`-diag`）复用这些检查的同一实现并给出分区结构化结论，但绝不写入或修复数据；正常启动的拒绝规则不因诊断模式放宽。进程在替换前中断保留旧快照，替换后中断使用新快照。同目录遗留的 `.strata-*` 临时文件不会参与恢复，可在服务停止时清理。若替换后同步目录失败，服务保留新内存状态并停止后续写入，健康状态变为 503；检查磁盘并重启后再读取版本确认结果，不要盲目重放修改。
...
**测试模式为 `deferred`**：当前初始化基线有意不生成单元测试、测试数据或专用测试套件；后续“代码测试”任务补充这些内容。`scripts/smoke.py` 是有超时的运行验证入口，它在临时目录编译并启动真实 HTTP 服务、通过本机回环 HTTP 连接完成操作，然后关闭服务并清理临时数据。不会访问外网或修改现有数据目录。也可分别运行 `record / seal / compare / browse / diagnose` 五个流程。

验证覆盖编录成功与深度失败边界、锁定与重新打开、并发版本冲突、历史不变性、数据目录独占、重启恢复、区间相似度、未知岩性、偏移建议、CSV、列表筛选与分页。`diagnose` 流程额外覆盖只读诊断在空目录、健康快照、校验值损坏、不受支持快照结构、版本链语义损坏下的结论一致性（与正常启动同一判断）、未执行区域为 `blocked`、致命失败叠加遗留临时文件时可恢复性仍为 `fail`、正文不泄露和持锁互斥。
