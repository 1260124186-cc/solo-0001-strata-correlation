...
| `-backups` | `STRATA_BACKUPS` | `<数据目录>/backups` | 备份文件目录 |
...
```bash
curl -sS -X POST http://127.0.0.1:8093/api/v1/backups \
  -H 'Content-Type: application/json' \
  -d '{"note":"锁定前备份"}'
```

响应为 HTTP 201，包含 `id`、`created_at`、`size_bytes` 和 `summary`（剖面数、版本数、对比数与 SHA-256 校验摘要）。备份在读锁保护下复制同一时刻的内存状态，包含全部剖面历史和对比结果，写入采用临时文件加原子替换，服务可继续处理请求。

```bash
curl -sS http://127.0.0.1:8093/api/v1/backups
curl -sS http://127.0.0.1:8093/api/v1/backups/<backup-id>
curl -sS http://127.0.0.1:8093/api/v1/backups/<backup-id>/preview
```

列表接口支持 `offset, limit` 分页，并标记每个备份文件是否完整。检查接口重新计算校验摘要并完整验证内容，报告 `checks` 与 `valid`。预览接口列出备份中的剖面与对比结果摘要，用于恢复前核对内容。

恢复必须显式确认：`confirm_id` 必须与路径中的备份编号一致，否则返回 HTTP 422。

```bash
curl -sS -X POST http://127.0.0.1:8093/api/v1/backups/<backup-id>/restore \
  -H 'Content-Type: application/json' \
  -d '{"confirm_id":"<backup-id>"}'
```

恢复先用验证过的备份内容整体替换磁盘快照，再替换内存状态；替换期间所有写入被阻止，不会与当前状态混写。提交点之前失败时内存和磁盘保持原资料；成功后内存、磁盘与健康状态一致。损坏的备份不能恢复，返回 HTTP 409 且现有数据不变。备份编号由服务生成，接口只接受编号；备份文件只写入 `-backups` 配置的目录，无法通过请求指定其他服务器路径。
...
| `POST /backups` | `note` 可选，创建在线恢复点 |
| `GET /backups` | `offset, limit` 分页列出备份 |
| `GET /backups/{id}` | 检查备份完整性与校验摘要 |
| `GET /backups/{id}/preview` | 预览备份中的剖面与对比 |
| `POST /backups/{id}/restore` | `confirm_id` 确认后整体恢复 |
...
单个剖面最多 500 层、500 个历史版本，总深度最大 1000000 毫米。最多 2000 个剖面、10000 个对比结果，总快照上限 64 MiB，每个备份文件同样限制 64 MiB。岩性支持 `sandstone / mudstone / limestone / shale / conglomerate / unknown`。名称最多 120 字、地点 200 字、说明 2000 字，单层描述 1000 字，标志层名称 80 字，修订理由 1–500 字，备份备注最多 500 字。标志层名称在同一剖面内忽略大小写后必须唯一。
...
该存储方案针对小规模资料，使用整份快照和内存副本，读写开销随历史体积增长。未实现数据删除、自动清理历史、备份轮替或多进程共享。日常备份使用在线恢复点接口（见“在线备份与恢复”），无需停止服务；备份目录中遗留的 `.strata-*` 临时文件同样不参与恢复。不要人工修改快照或备份内容。
...
**测试模式为 `deferred`**：当前初始化基线有意不生成单元测试、测试数据或专用测试套件；后续“代码测试”任务补充这些内容。`scripts/smoke.py` 是有超时的运行验证入口，它在临时目录编译并启动真实 HTTP 服务、通过本机回环 HTTP 连接完成操作，然后关闭服务并清理临时数据。不会访问外网或修改现有数据目录。也可分别运行 `record / seal / compare / browse / backup` 五个流程。

验证覆盖编录成功与深度失败边界、锁定与重新打开、并发版本冲突、历史不变性、数据目录独占、重启恢复、区间相似度、未知岩性、偏移建议、CSV、列表筛选与分页、在线备份的创建列出检查预览、确认恢复、并发恢复隔离和损坏备份拒绝。
