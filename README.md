...
单个剖面最多 500 层、500 个历史版本，总深度最大 1000000 毫米。最多 2000 个剖面、10000 个对比结果，单个分片上限 64 MiB（旧整份快照同样上限 64 MiB，仅迁移时读取）。岩性支持 `sandstone / mudstone / limestone / shale / conglomerate / unknown`。名称最多 120 字、地点 200 字、说明 2000 字，单层描述 1000 字，标志层名称 80 字，修订理由 1–500 字。标志层名称在同一剖面内忽略大小写后必须唯一。

差异接口按完整深度区间匹配分层；边界变化展示为原区间移除和新区间增加，相同区间中的岩性或描述修改展示前后值。深度查询使用左闭右开区间，边界点属于其下方分层，剖面底端不属于任何层。

## 持久化与恢复

数据目录保存 `strata.lock` 和分片目录 `shards/`。每个剖面的完整版本历史保存为 `shards/prf_<编号>.json`，每条对比结果保存为 `shards/cmp_<编号>.json`。一次写入只把被改动的那一份数据写到分片目录中的临时文件，执行文件 fsync 后原子替换对应分片，再对目录执行 fsync，写入耗时不再随整体历史线性增长。失败的校验不会修改内存或磁盘。

每个分片是带 `format / kind / id / digest / data` 的信封，`digest` 为数据部分的 SHA-256，且文件名必须与内部编号一致。启动时先清扫根目录和分片目录中遗留的 `.strata-*` 临时文件，再逐个读取分片：分片必须命名合法、信封完整、校验通过，分片目录中出现任何其他文件或子目录都会拒绝启动。全部分片装配后统一检查版本连续性、状态转换及对比可重复性，因此任意时刻被杀、只写完一部分内容或目录同步失败，重启后都恢复出唯一确定的状态。

旧版本的整份快照 `strata.json` 可以直接被读起：启动时读取并校验旧快照后，按剖面和对比逐分片原子写出，最后删除旧快照。迁移每一步都可重放——迁移过程中被杀，重启后旧快照仍在则重新覆盖写出对应分片，已删除则直接使用分片形态，两种情况收敛到同一状态。读取接口与 CSV 导出在迁移前后保持不变。

若分片替换成功但随后同步目录失败，或一次提交中前一分片已落地、后续分片失败，服务保留最后一个完整一致的内存状态并停止后续写入，健康状态变为 503；磁盘与内存是否一致以该故障状态为准，重启后从磁盘重新装配并确认结果，不要盲目重放修改。

该存储方案针对小规模资料，使用分片文件和全量内存副本，写入开销只与被改动剖面的历史大小相关。未实现数据删除、自动清理历史、备份轮替或多进程共享。数据目录仍由单个进程通过文件锁独占，锁在进程退出（包括异常退出）后由操作系统释放。备份时停止服务后复制数据目录；不要人工修改分片内容。
...
**测试模式为 `deferred`**：当前初始化基线有意不生成单元测试、测试数据或专用测试套件；后续“代码测试”任务补充这些内容。`scripts/smoke.py` 是有超时的运行验证入口，它在临时目录编译并启动真实 HTTP 服务、通过本机回环 HTTP 连接完成操作，然后关闭服务并清理临时数据。不会访问外网或修改现有数据目录。也可分别运行 `record / seal / compare / browse / migrate` 五个流程。

验证覆盖编录成功与深度失败边界、锁定与重新打开、并发版本冲突、历史不变性、数据目录独占、重启恢复、旧整份快照迁移与迁移中断重放、区间相似度、未知岩性、偏移建议、CSV、列表筛选与分页。
...
internal/persistence/分片校验、原子替换、快照迁移、独占锁
