...
func New(service *catalog.Service, logger *slog.Logger) http.Handler {
	...
	mux.HandleFunc("POST /api/v1/comparisons/{id}/credential", h.issueCredential)
	mux.HandleFunc("GET /api/v1/comparisons/{id}/credential", h.comparisonCredential)
	mux.HandleFunc("GET /api/v1/credentials/{id}", h.getCredential)
	mux.HandleFunc("POST /api/v1/credentials/{id}/revoke", h.revokeCredential)
	mux.HandleFunc("GET /api/v1/revocation-list", h.crl)
	mux.HandleFunc("GET /api/v1/signing-key", h.signingKey)
