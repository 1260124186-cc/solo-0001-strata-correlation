...
func New(service *catalog.Service, logger *slog.Logger) http.Handler {
	...
	mux.HandleFunc("POST /api/v1/derivations/preview", h.derivePreview)
	mux.HandleFunc("POST /api/v1/derivations", h.deriveConfirm)
	mux.HandleFunc("GET /api/v1/profiles/{id}/derivation", h.derivation)
	mux.HandleFunc("GET /api/v1/profiles/{id}/lineage", h.lineage)
