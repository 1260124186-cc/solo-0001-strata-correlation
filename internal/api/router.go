...
func New(service *catalog.Service, logger *slog.Logger) http.Handler {
	...
	mux.HandleFunc("POST /api/v1/profiles/{id}/fork", h.fork)
	mux.HandleFunc("GET /api/v1/profiles/{id}/branches", h.branches)
	mux.HandleFunc("GET /api/v1/profiles/{id}/branches/{branch}/relation", h.branchRelation)
	mux.HandleFunc("POST /api/v1/profiles/{id}/merge-preview", h.mergePreview)
	mux.HandleFunc("POST /api/v1/profiles/{id}/merge", h.merge)
