...
每次写入使用预期版本；同一版本的并发请求仅一个成功。片段式分层替换整体提交，校验失败不改变任何层。剖面每个版本和事件作为一个持久化单元原子写入磁盘。数据目录同时只能有一个写入进程；允许额外只读进程挂载同一路径，跟随最近一次成功提交，且不会读到替换中的快照。异常退出由操作系统释放锁。
...
## 容量与运行边界
最多 2000 个剖面、每个剖面 500 个版本、10000 个对比结果，持久化快照最多 64 MiB。当前基线适用于 Linux/macOS 单写入者、多只读进程的本机服务，无外部依赖、无前端、无数据删除和多节点复制。正式测试套件由后续专项测试任务添加。
