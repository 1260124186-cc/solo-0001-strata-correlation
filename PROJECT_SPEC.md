...
每次写入使用预期版本；同一版本的并发请求仅一个成功。写入因版本或状态冲突被拒绝时，响应附带结构化的当前状态（期望版本、当前版本、当前状态、内容稳定指纹和重新读取定位），服务不自动合并或重试，冲突响应不产生版本、事件或快照写入。片段式分层替换整体提交，校验失败不改变任何层。剖面每个版本和事件作为一个持久化单元原子写入磁盘。单进程独占数据目录，异常退出由操作系统释放锁。
