...
## 实体和规则
剖面包含名称、地点描述、总深度（整数毫米）、可选资料来源、分层数组、版本和状态。分层包含顶底深度、岩性、描述、可选标志层。支持砂岩、泥岩、灰岩、页岩、砾岩和未知岩性。深度上限一千米，每个剖面最多 500 层；文字字段有明确长度限制。
...
快照按结构版本解码：新写入使用当前版本，旧版本快照读取时校验并迁移到当前形态；未知字段或未知版本拒绝启动，不静默丢弃。
