...
每次写入使用预期版本；同一版本的并发请求仅一个成功，失败响应附带预期版本以来已变化的字段。元数据支持完整替换与局部更新：局部更新中未出现的字段保持原值，说明可显式清空，名称与地点不允许清空；两种入口共用同一套版本校验、锁定边界与事件语义。片段式分层替换整体提交，校验失败不改变任何层。剖面每个版本和事件作为一个持久化单元原子写入磁盘。单进程独占数据目录，异常退出由操作系统释放锁。
